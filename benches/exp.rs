//! Comprehensive Exponential Benchmarks with SIMD vs Scalar Comparison
//!
//! This benchmark suite evaluates the performance of different exponential (e^x) implementations
...
//! - Pure scalar implementation using standard library exp (baseline)
...
// No fast_exp function available

// Import the exponential implementations
...
/// # Returns
///
/// A vector containing random f32 values in range [-5.0, 5.0] for meaningful exp input
fn generate_test_data(len: usize) -> Vec<f32> {
    let mut rng = StdRng::seed_from_u64(42); // Fixed seed for reproducibility

    (0..len)
        .map(|_| (rng.random::<f32>() - 0.5) * 10.0) // Range [-5, 5] to avoid overflow
        .collect()
}
...
/// Benchmarks all exponential implementations across different vector sizes.
fn benchmark_exp_implementations(c: &mut Criterion) {
    for &size in VECTOR_SIZES {
        let mut group = c.benchmark_group(format!("Exponential {}", format_size(size)));
        ...
        // Benchmark 1: Scalar Exponential (Baseline)
        group.bench_with_input(BenchmarkId::new("Scalar", size), input_slice, |b, input| {
            b.iter(|| black_box(input.iter().map(|x| x.exp()).collect::<Vec<f32>>()))
        });

        // Benchmark 2: SIMD Exponential
        group.bench_with_input(BenchmarkId::new("SIMD", size), input_slice, |b, input| {
            b.iter(|| black_box(input.exp()))
        });

        // Benchmark 3: Parallel SIMD Exponential
        group.bench_with_input(
            BenchmarkId::new("Parallel SIMD", size),
            input_slice,
            |b, input| b.iter(|| black_box(black_box(input).par_exp())),
        );

        // Note: No fast_exp function available in the library
...
fn all_benchmarks(c: &mut Criterion) {
    println!("🚀 Starting Comprehensive Exponential Benchmarks");
    println!(
    ...
    benchmark_exp_implementations(c);
