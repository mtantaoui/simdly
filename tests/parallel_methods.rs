...
fn test_parallel_abs_large_array() {
    ...
    for (i, &val) in result.iter().enumerate() {
        ...
        assert!(
            ...
            "par_abs large array failed at index {}: {} != {}",
            i,
            val,
            expected
        );
...
fn test_parallel_cos_methods() {
    ...
    assert!(
        (result_vec[0] - 1.0).abs() < 1e-5,
        "Vec cos(0) should be ~1.0"
    );
}
...
fn test_parallel_exp_methods() {
    ...
    assert!(
        (result_slice[1] - std::f32::consts::E).abs() < 1e-4,
        "exp(1) should be ~e"
    );
...
fn test_parallel_floor_ceil_methods() {
    ...
    let expected_floor = [1.0f32, 2.0, -2.0, -3.0];
    let expected_ceil = [2.0f32, 3.0, -1.0, -2.0];
