//! Comprehensive Square Root Benchmarks with SIMD vs Scalar Comparison
//!
//! This benchmark suite evaluates the performance of different square root implementations
...
// No fast_sqrt function available

// Import the square root implementations
...
/// # Returns
///
/// A vector containing random f32 values in range [0.0, 1000.0) for meaningful sqrt input
fn generate_test_data(len: usize) -> Vec<f32> {
    let mut rng = StdRng::seed_from_u64(42); // Fixed seed for reproducibility

    (0..len)
        .map(|_| rng.random::<f32>() * 1000.0) // Range [0, 1000) - all positive for sqrt
        .collect()
}
...
/// Benchmarks all square root implementations across different vector sizes.
fn benchmark_sqrt_implementations(c: &mut Criterion) {
    for &size in VECTOR_SIZES {
        ...
        // Benchmark 1: Scalar Square Root (Baseline)
        group.bench_with_input(BenchmarkId::new("Scalar", size), input_slice, |b, input| {
            b.iter(|| black_box(input.iter().map(|x| x.sqrt()).collect::<Vec<f32>>()))
        });

        // Benchmark 2: SIMD Square Root
        group.bench_with_input(BenchmarkId::new("SIMD", size), input_slice, |b, input| {
            b.iter(|| black_box(input.sqrt()))
        });

        // Benchmark 3: Parallel SIMD Square Root
        group.bench_with_input(
            BenchmarkId::new("Parallel SIMD", size),
            input_slice,
            |b, input| b.iter(|| black_box(black_box(input).par_sqrt())),
        );

        // Note: No fast_sqrt function available in the library
...
fn all_benchmarks(c: &mut Criterion) {
    println!("🚀 Starting Comprehensive Square Root Benchmarks");
    println!(
    ...
    benchmark_sqrt_implementations(c);
