//! Comprehensive Absolute Value Benchmarks with SIMD vs Scalar Comparison
//!
//! This benchmark suite evaluates the performance of different absolute value implementations
...
//! - Pure scalar implementation using standard library abs (baseline)
...
// No fast_abs function available

// Import the absolute value implementations
...
/// # Returns
///
/// A vector containing random f32 values including negative values for meaningful abs input
fn generate_test_data(len: usize) -> Vec<f32> {
    let mut rng = StdRng::seed_from_u64(42); // Fixed seed for reproducibility

    (0..len)
        .map(|_| rng.random::<f32>() * 200.0 - 100.0) // Range [-100, 100] with negatives
        .collect()
}
...
/// Benchmarks all absolute value implementations across different vector sizes.
fn benchmark_abs_implementations(c: &mut Criterion) {
    for &size in VECTOR_SIZES {
        let mut group = c.benchmark_group(format!("Absolute Value {}", format_size(size)));
        ...
        // Benchmark 1: Scalar Absolute Value (Baseline)
        group.bench_with_input(BenchmarkId::new("Scalar", size), input_slice, |b, input| {
            b.iter(|| black_box(input.iter().map(|x| x.abs()).collect::<Vec<f32>>()))
        });

        // Benchmark 2: SIMD Absolute Value
        group.bench_with_input(BenchmarkId::new("SIMD", size), input_slice, |b, input| {
            b.iter(|| black_box(input.abs()))
        });

        // Benchmark 3: Parallel SIMD Absolute Value
        group.bench_with_input(
            BenchmarkId::new("Parallel SIMD", size),
            input_slice,
            |b, input| b.iter(|| black_box(black_box(input).par_abs())),
        );

        // Note: No fast_abs function available in the library
...
fn all_benchmarks(c: &mut Criterion) {
    println!("🚀 Starting Comprehensive Absolute Value Benchmarks");
    println!(
    ...
    benchmark_abs_implementations(c);
