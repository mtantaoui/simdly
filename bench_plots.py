...
from bokeh.models import (
    HoverTool, ColumnDataSource, NumeralTickFormatter, FactorRange,
    Legend, LegendItem
)
...
def plot_line_chart_bokeh(group_name, df_group):
    ...
    df_group['Throughput (GiB/s)'] = (df_group['Size_Bytes'] / (df_group['Time (ns)'] * 1e-9)) / GIB_CONVERSION_FACTOR
    df_group['Time_ms'] = df_group['Time (ns)'] / 1_000_000
    df_group['Size_Label_Tooltip'] = df_group['Size_Bytes'].apply(format_bytes_for_label)

    output_filename = os.path.join(PLOT_OUTPUT_DIR, f"{group_name.replace(' ', '_')}_line_performance.html")
    ...
    for i, impl_name in enumerate(implementations):
        source_df = df_group[df_group['Implementation'] == impl_name].sort_values(by='Size')
        source = ColumnDataSource(source_df)
        color = palette[i % len(palette)]
        p.line(x='Size', y='Time_ms', source=source, legend_label=impl_name, line_width=2, color=color)
        p.circle(x='Size', y='Time_ms', source=source, legend_label=impl_name, size=8, color=color, alpha=0.8)
...
def plot_grouped_bar_chart_per_size_bokeh(group_name, df_group_for_size, size_elements, size_label):
    ...
    implementations_sorted = df_plot['Implementation'].tolist() # Use sorted order for x-axis
    ...
    p = figure(
        x_range=FactorRange(*implementations_sorted), # Implementations are the x-categories
        height=400,
        width=max(600, len(implementations_sorted) * 80), # Adjust width based on number of implementations
        title=f"Execution Time for {group_name} (Size: {size_label})",
    ...
    palette = Bright6 if len(implementations_sorted) <= 6 else Category10[max(3, len(implementations_sorted))]

    p.vbar(
        x='Implementation', top='Time_ms', source=source, width=0.7,
        # Color by implementation
        fill_color=factor_cmap('Implementation', palette=palette, factors=implementations_sorted),
        line_color="black",
        # No legend needed if colors are directly mapped and x-axis shows implementation
        # Or, if you want a legend for colors:
        # legend_field="Implementation" # This would create a legend based on the 'Implementation' column
    )
    # If using legend_field, you might want to customize the legend:
...
def main():
    ...
    for group_name, df_group_data in df_all.groupby("Group"):
        ...
        for size_elements, df_single_size_data in df_group_data.groupby("Size"):
            size_bytes_val = df_single_size_data['Size_Bytes'].iloc[0] # Get Size_Bytes for this group
            size_label_str = format_bytes_for_label(size_bytes_val)
            plot_grouped_bar_chart_per_size_bokeh(
                group_name,
                df_single_size_data.copy(),
                size_elements,
                size_label_str
            )
