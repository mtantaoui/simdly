...
pub(crate) mod float32 {
    ...
    /// Computes `acos(d)` for 16 floats using the identity `acos(x) = π/2 - asin(x)`.
    #[inline(always)]
    pub unsafe fn _mm512_acos_ps(d: __m512) -> __m512 {
        let pi_2 = _mm512_set1_ps(std::f32::consts::FRAC_PI_2);

        // Reuse our high-quality AVX-512 asin implementation.
        let asin_d = _mm512_asin_ps(d);

        // Return π/2 - asin(d).
        _mm512_sub_ps(pi_2, asin_d)
    }
