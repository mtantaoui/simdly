//! Comprehensive Tangent Benchmarks with SIMD vs Scalar Comparison
//!
//! This benchmark suite evaluates the performance of different tangent implementations
...
//! - Pure scalar implementation using standard library tangent (baseline)
...
/// # Returns
///
/// A vector containing random f32 values in range (-π/2, π/2) for meaningful tangent input
fn generate_test_data(len: usize) -> Vec<f32> {
    let mut rng = StdRng::seed_from_u64(42); // Fixed seed for reproducibility

    (0..len)
        .map(|_| (rng.random::<f32>() - 0.5) * std::f32::consts::PI * 0.95) // Range (-π/2, π/2) avoiding singularities
        .collect()
}
...
/// Benchmarks all tangent implementations across different vector sizes.
fn benchmark_tan_implementations(c: &mut Criterion) {
    for &size in VECTOR_SIZES {
        let mut group = c.benchmark_group(format!("Tangent {}", format_size(size)));
        ...
        // Benchmark 1: Scalar Tangent (Baseline)
        group.bench_with_input(BenchmarkId::new("Scalar", size), input_slice, |b, input| {
            b.iter(|| black_box(input.iter().map(|x| x.tan()).collect::<Vec<f32>>()))
        });

        // Benchmark 2: SIMD Tangent
        group.bench_with_input(BenchmarkId::new("SIMD", size), input_slice, |b, input| {
            b.iter(|| black_box(input.tan()))
        });

        // Benchmark 3: Parallel SIMD Tangent
        group.bench_with_input(
            BenchmarkId::new("Parallel SIMD", size),
            input_slice,
            |b, input| b.iter(|| black_box(black_box(input).par_tan())),
        );

        // Note: No fast_tan function available in the library
...
fn all_benchmarks(c: &mut Criterion) {
    println!("🚀 Starting Comprehensive Tangent Benchmarks");
    println!(
    ...
    benchmark_tan_implementations(c);
