//! Precision comparison tests between scalar and SIMD arccosine implementations.
//!
//! This test suite validates that the SIMD arccosine implementation maintains
...
/// Test precision of SIMD acos against scalar acos for various input ranges.
#[test]
fn test_acos_precision_comparison() {
    // Test cases covering the valid domain [-1, 1] for acos
    let test_cases = vec![
        // Values near zero
    ...
    for (i, test_case) in test_cases.iter().enumerate() {
        ...
        let scalar_results: Vec<f32> = test_case.iter().map(|x| x.acos()).collect();

        // Compute SIMD results
        let simd_results = test_case.acos();
            ...
            scalar_results.iter().zip(simd_results.iter()).enumerate()
        {
            ...
            // acos can be sensitive near the boundaries
            assert!(
...
#[test]
fn test_acos_precision_random_inputs() {
    ...
    // Generate random inputs in valid domain [-1, 1] for acos
    let inputs: Vec<f32> = (0..test_size)
        .map(|_| rng.random_range(-1.0..=1.0))
        .collect();

    let scalar_results: Vec<f32> = inputs.iter().map(|x| x.acos()).collect();
    let simd_results = inputs.acos();
    ...
    for (i, (&scalar_val, &simd_val)) in scalar_results.iter().zip(simd_results.iter()).enumerate()
    {
        ...
        // Individual assertion with reasonable tolerance for acos
        assert!(
...
fn test_acos_edge_cases() {
    ...
    for (input, expected) in edge_cases {
        ...
        let scalar_result = input_vec.iter().map(|x| x.acos()).collect::<Vec<f32>>()[0];
        let simd_result = input_vec.acos()[0];

        println!("Edge case: acos({input:.6}) = {scalar_result:.8} (expected: {expected:.8})");
        ...
        if expected != 0.0 {
            assert!(
                ...
                "Scalar acos relative error too large for {input}: {scalar_rel_error:.2e}"
            );
            assert!(
                simd_rel_error < 1e-4,
                "SIMD acos relative error too large for {input}: {simd_rel_error:.2e}"
            );
        } else {
            assert!(
                scalar_error < 1e-6,
                "Scalar acos absolute error too large for {input}: {scalar_error:.2e}"
            );
            assert!(
                simd_error < 1e-5,
                "SIMD acos absolute error too large for {input}: {simd_error:.2e}"
            );
        }
...
/// Test precision with values near the boundaries where acos is most sensitive.
#[test]
fn test_acos_precision_near_boundaries() {
    ...
    let simd_results = boundary_values.acos();
    ...
    for (i, (&scalar_val, &simd_val)) in scalar_results.iter().zip(simd_results.iter()).enumerate()
    {
        ...
        println!(
            "Boundary value: acos({input_val:.6}) -> Scalar: {scalar_val:.8}, SIMD: {simd_val:.8}, Error: {absolute_error:.2e}, Rel: {relative_error:.2e}"
        );

        // SIMD should match scalar closely, even near boundaries
        if scalar_val != 0.0 {
            assert!(
                relative_error < 1e-4,
                "SIMD precision error too large for boundary input {input_val}: {relative_error:.2e}"
            );
        } else {
            assert!(
                absolute_error < 1e-5,
