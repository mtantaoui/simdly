...
pub(crate) unsafe fn kernel_8x8(
    a_panel_data: *const f32,
    b_panel_data: *const f32,
    ...
    for k in 0..kc {
        ...
        let a_col_ptr = a_panel_data.add(k * MR);
        let b_row_ptr = b_panel_data.add(k * NR);
        
        let a_micropanel = F32x8::load_aligned(a_col_ptr);
        let b_micropanel = F32x8::load_aligned(b_row_ptr);
